...
async def get_student_info(session: Session = Depends(get_authenticated_session)):
    """
    ## 业务说明
    ...
    - 数据来源于教务系统，字段名为拼音缩写
    """
    return await grades_service.get_student_info(session)


@router.get("/user-info", response_model=dict, summary="获取用户完整信息")
async def get_user_info(session: Session = Depends(get_authenticated_session)):
    """
    ## 业务说明
    ...
    """
    return await grades_service.get_user_info(session)


@router.get("/student-plan", response_model=list, summary="获取学生培养方案")
async def get_student_plan(session: Session = Depends(get_authenticated_session)):
    """
    ## 业务说明
    ...
    - 不同专业的培养方案结构可能不同
    """
    return await grades_service.get_student_plan(session)


@router.get("/available-terms", response_model=dict, summary="查询可选学年学期")
async def get_available_terms(session: Session = Depends(get_authenticated_session)):
    """
    ## 业务说明
    ...
    - 返回的学期列表是动态的，会随着时间更新
    """
    return await grades_service.get_available_terms(session)
...
async def get_term_list(session: Session = Depends(get_authenticated_session)):
    ...
    - 需要先登录认证
    """
    return await grades_service.get_term_list(session)


@router.get("/credit-completion-status", response_model=dict, summary="获取学分完成情况")
async def get_credit_completion_status(session: Session = Depends(get_authenticated_session)):
    """
    ## 业务说明
    ...
    - 统计基于培养方案和已修课程
    """
    return await grades_service.get_credit_completion_status(session)
